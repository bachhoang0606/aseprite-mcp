...
- Session/sprite: `live_list_sprites`, `live_open_sprite`, `live_activate_sprite`, `live_get_active_site`, `live_get_sprite_info`, `live_set_sprite_properties`, `live_save_sprite`, `live_save_sprite_as`, `live_save_copy_as`, `live_save_preview`, `live_ascii_view`, `live_save_filmstrip`, `live_frame_diff`, `live_close_sprite`, `live_resize_canvas`.
  ...
  - `live_frame_diff` returns a **pixel-level diff** of two frames as a text grid: `.` = unchanged, `-` = erased (became transparent), otherwise the glyph of the **new** colour at that cell (with a glyph→`#rrggbb` legend) and a changed-cell count. Pass 1-based `from_frame`/`to_frame`. Use it to confirm EXACTLY what an edit changed, or to inspect motion between two animation frames at the pixel level (the *verify* half of draw→see→fix). Restores the user's active frame; refuses sprites > 64×64 (crop first).
